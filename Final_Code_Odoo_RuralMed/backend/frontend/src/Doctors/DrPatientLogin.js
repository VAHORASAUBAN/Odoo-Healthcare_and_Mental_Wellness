...
function Login() {
  ...
  // Handle login form submission with XMLHttpRequest
  const handleSubmit = (e) => {
    ...
    xhr.open('POST', 'http://localhost:8000/api/login/', true); // URL to the backend Django API
      ...
      if (xhr.readyState === 4) {  // When request is complete
        try {
          ...
          if (xhr.status === 200) {
            ...
            navigate('/Patient/Dashboard');  // Redirect to dashboard on successful login
          } else {
            // Show error message if login fails
            alert(response.error || 'An error occurred during login.');
          }
        } catch (error) {
          // Log the full response to debug the error
    ...
    const data = JSON.stringify({
      email: email,  // Ensure the field name is correct
      password: password,  // Ensure the field name is correct
    });
