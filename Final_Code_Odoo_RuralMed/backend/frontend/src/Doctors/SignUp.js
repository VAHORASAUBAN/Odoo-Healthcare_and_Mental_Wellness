...
const SignUp = () => {
  ...
  const handleSubmit = (e) => {
    ...
    xhr.open('POST', 'http://localhost:8000/api/signup/', true); // Replace with your actual Django endpoint

    // Set the request header for JSON content
    xhr.setRequestHeader('Content-Type', 'application/json');

    // Define what to do when the request finishes (success or error)
    xhr.onload = () => {
      if (xhr.status === 200) {
        // On success, redirect to the login page
        alert('Sign-up successful!');
        navigate('/login'); // Programmatically redirect to login page
      } else {
        // Handle error
